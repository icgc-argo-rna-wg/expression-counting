...
df_inp = pd.read_csv(args.input,sep='\t')
...
# TPM
df_TPM = df_readCounts.apply(lambda x:10**3*x/df_readCounts.index.map(dict_length)).dropna().apply(lambda x:x/sum(x)).applymap(lambda x:x*10**6).rename(columns = {"readCounts":'TPM'})
